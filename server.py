...
def client_thread(conn):
    ...
    connected = True
    client_name = None
    clients_connected[conn] = client_name
    ...
    while connected: 
        ...
        # if command exists and length of command_args != to required number of arguments, send error message
            ...
            if command_args[0] in ['/message', '/broadcast'] and len(command_args) > 30:
                conn.sendall('Error: 30 characters limit exceeded.'.encode())
                continue
            else:
                conn.sendall('Error: Command parameters mismatch.'.encode())
                continue
        ...
        if not (client_name in clients_registered):
            ...
            if command_args[0] not in ['/register', '/leave', '/?']:
                conn.sendall('Error: Please register a name first.'.encode())
                continue
        else:
            # if already registered and command is /register, send error message
            if command_args[0] in ['/register']:
                conn.sendall('Error: You have already registered a name.'.encode())
                continue

        match command_args[0]:
            case '/leave':
                conn.sendall(f"Connection closed.".encode())
                broadcast_message(f"{client_name} has left the server.")

                # remove client
                clients_connected.pop(conn)
            ...
            case '/register':
                ...
                if client_name is None:
                    conn.sendall('Error: Please enter a valid username.'.encode())
                elif client_name in clients_registered:
                    conn.sendall('Error: Username already taken.'.encode())
                else:
                    clients_connected[conn] = client_name
                    clients_registered.add(client_name)
                    conn.sendall(f"Successfully registered as {client_name}.".encode())
                    broadcast_message(f"{client_name} has joined the server.")

            case '/dir': # list all files in server_dir
                files = os.listdir('server_dir')
                conn.sendall("Server Directory:\n".encode()+"\n".join(files).encode())
            ...
            case '/store': # from client_dir to server_dir
                ...
                conn.sendall(f"{client_name}<{readable_time}>: Uploaded {file_name}".encode())
            ...
            case '/get': # from server_dir to client_dir
                ...
                if file_name not in files:
                    conn.sendall(b'0') # send 0 if file does not exist
                    conn.sendall(f"Error: {file_name} does not exist in the server.".encode())
                    continue
                
                with open(f"server_dir/{file_name}", 'rb') as f:
                    file_data = f.read()
                    conn.sendall(b'1') # send 1 if file exists
